...
/************************************************************************/
/* 查看盘符是否可以使用，没有被其他设备占用  （必须是大写字母）
* 输入参数:
*		chVailLetter: 盘符
* 返回值：
*	 	TRUE--盘符未被占用，可以使用；FALSE--盘符不能使用
/************************************************************************/
SAGE_API BOOL __stdcall IsLetterValidateW_SageAPI( WCHAR wVailLetter );
SAGE_API BOOL __stdcall IsLetterValidateA_SageAPI( CHAR chVailLetter );
#ifdef UNICODE
#define IsLetterValidate_SageAPI  IsLetterValidateW_SageAPI
#else
#define IsLetterValidate_SageAPI  IsLetterValidateA_SageAPI
#endif
...
*		_In chMountLetter : 安全区挂载盘符
* 返回值：
*	 	0--成功；非0--失败  1--盘符不可用  2--创建挂载进程失败
/************************************************************************/
SAGE_API UINT32 __stdcall MountSecurityDiskW_SageAPI( const WCHAR *wMountFilePath , const MountSecurityDiskParam aMountParam , WCHAR chMountLetter ) ;
