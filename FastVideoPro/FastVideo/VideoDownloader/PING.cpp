...
void CPing::ScanIp(vector<QString> &vecIp, bool bArp)
{
	vector<string> vecIpHost;
	GetLocalIp(vecIpHost);
    for (vector<string>::iterator itr = vecIpHost.begin(); itr != vecIpHost.end(); itr++)
	{
        ScanIPs(vecIp, itr->c_str(), bArp);
	}
}

void CPing::ScanIPs(vector<QString> &vecIPs, const QString& netIp, bool bArp){

    unsigned long ulIp = inet_addr(netIp.toStdString().c_str());
    in_addr addr;
    addr.s_addr = ulIp;
    ...
    for (unsigned char i = 2; i < 255; ++i)
    {
        ...
        {
            vecIPs.push_back(QString(strIpTmp.c_str()));
        }
    }
}

bool CPing::ScanOneIP(QString &ip, const QString& netIp, bool bArp){
    QString netPre = netIp.left(netIp.lastIndexOf(".") + 1);
    for (int i = 1; i < 255; i++)
    {
        ip = netPre + QString::number(i);
        qDebug()<<ip;
        if (netIp != ip && Ping((char *)ip.toStdString().c_str(), 20, bArp))//Ping((char *)strIpTmp.c_str(), &reply, 2000)
        {
             return true;
        }
    }
...
BOOL CPing::PingCore(DWORD dwDestIP, long dwTimeout, std::shared_ptr<OnePingInfo> pInfo, bool bArp)
{
	...
	{
        qDebug() << __FILE__ << __FUNCTION__ << __LINE__ << "sendto";
		return FALSE;
	}
    ...
    for (ULONG ulTimeOutTick = GetTickCountCalibrate() + dwTimeout; dwTimeout > 0; dwTimeout = ulTimeOutTick - GetTickCountCalibrate())
    {
        ...
        if (nPacketSize == SOCKET_ERROR){
            qDebug() << __FILE__ << __FUNCTION__ << __LINE__ << "SOCKET_ERROR";
            continue;
        }
