    ...
    __time64_t timeEnd)
{
   ...
   /* if (m_lLoginHandle <= 0)
	...
	for (struStartTime.dwDay; struStartTime.dwDay <= struStopTime.dwDay; struStartTime.dwDay += 1){
		for (int ch = 0; ch < channelVec.size(); ch++)
		{
			...
			/*if (ch != 0){
				struStartTime.dwDay = struStartTime.dwDay - m_days;
				}*/

			LONG lfind = DVR_FindFile(m_lLoginHandle, nChannelId, rt_all, &struStartTime, &struStopTime);			//////////////////////////////////////////////////返回1
			...
			LONG re = DVR_FindNextFile(lfind, &findInfo);////////////////////////////////////////////////////////////////////////////////////////////////////finfInfo.dwFilesize = 0



			char ddwdd[500] = { 0 };
			sprintf_s(ddwdd, "xxxxxxwww111xxxxx s_time:%d-%02d-%02d %02d:%02d:%02d e_time:%d-%02d-%02d %02d:%02d:%02d", findInfo.struStartTime, findInfo.struStopTime);
			Log::instance().AddLog(string(ddwdd));

			/*	char ssttrwwr[32] = { 0 };
				sprintf_s(ssttrwwr, "dwFileSize:%d", findInfo.dwFileSize);
				Log::instance().AddLog(string(ssttrwwr));*/
			...
			while (true)
			{
				...
				/*if (struStartTime.dwDay < struStopTime.dwDay){
					struStartTime.dwDay += 1;
					lfind = DVR_FindFile(m_lLoginHandle, nChannelId, rt_all, &struStartTime, &struStopTime);
					}*/
