...
{
	unsigned long	deviceID;				//设备ID(暂时未用上)
	...
	unsigned long	manufacturerId;			//设备厂商ID(暂时未用上)
...
NET_SDK_API BOOL CALL_METHOD NET_SDK_PTZControl_3D(LONG lLiveHandle, LONG lChannel, PTZ_3D_POINT_INFO *pPtz3DInfo);
...
NET_SDK_API LONG CALL_METHOD NET_SDK_GetFileByTimeEx(LONG lUserID,LONG lChannel, DD_TIME * lpStartTime, DD_TIME * lpStopTime, char *sSavedFileName, BOOL bCustomFormat); //bCustomFormat：是否使用私有文件格式
