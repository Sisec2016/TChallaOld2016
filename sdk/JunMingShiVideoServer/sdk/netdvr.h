...
enum NETDVR_RETURN_CODE	//
{
	...
	//LTNET
	NETDVR_ERR_OFFLINEUPNP,
	NETDVR_ERR_UPNP,
	NETDVR_ERR_VVSERVERNOCON,
	NETDVR_ERR_VVSDKINIT,
	NETDVR_ERR_KEYINVALID,
	NETDVR_ERR_OFFLINEVV,
	NETDVR_ERR_CHECKSTATUS,
	NETDVR_ERR_VVADDPORT,
	NETDVR_ERR_PORTSTATUSTIMEOUT,

	NETDVR_ERR_SENDCONFIGEFILE,
...
struct NETDVR_DeviceInfo_t
{
	...
	unsigned char	reserved[16];					//预留reserved[0]是用来标示字符集的,reserved[1]1、2、3分别有定制项
};
...
struct NETDVR_SubStreamParam_t								
{
	...
	unsigned int		gop; // add by wiat
	unsigned char		reserved[12];				//保留字段 // modify by wait
};
...
#define MAX_CRUISE_PATH_NUM 32				//max cruise paths of each PTZ
#define MAX_CRUISE_POS_NUM 128				//max cruise positions of each cruise path
...
enum NETDVR_PTZCONTROL {
	...
	NETDVR_PTZ_COM_ZOOMADDSTOP = 21,
	NETDVR_PTZ_COM_ZOOMSUBTRACTSTOP = 22,
	NETDVR_PTZ_COM_FOCUSADDSTOP = 23,
	NETDVR_PTZ_COM_FOCUSSUBTRACTSTOP = 24,
};
...
typedef enum NETDVR_PRERECORD_TIME {
	NETDVR_PRERECORD_TIME_0 = 0,		
	NETDVR_PRERECORD_TIME_1 = 1,		
	NETDVR_PRERECORD_TIME_2 = 2,		
	NETDVR_PRERECORD_TIME_3 = 3,		
	NETDVR_PRERECORD_TIME_4 = 4,		
	NETDVR_PRERECORD_TIME_5 = 5,		
	NETDVR_PRERECORD_TIME_6 = 6,		
	NETDVR_PRERECORD_TIME_7 = 7,		
	NETDVR_PRERECORD_TIME_8 = 8,		
	NETDVR_PRERECORD_TIME_9 = 9,		
	NETDVR_PRERECORD_TIME_10 = 10	
} prerecord_time_t;
...
{	
	...
	unsigned int		gop; // add by wait
	unsigned char		reserved1[8];				//保留字段	 // modify by wait
};
...
struct NETDVR_alarmInParam_t
{
	...
	unsigned char		chntobezoomed;				//放大通道
	unsigned char		reserved[14];				//保留字段
};
...
struct NETDVR_AlarmUploadState_t
{
	...
	//5-读写硬盘出错,6-遮挡报警,7-制式不匹配, 8-非法访问, 9-设备重启, 10-硬盘异常
	unsigned char	type;		
...
{
	struct NETDVR_userInfo_t userinfo[50];			//最多8个用户
};

struct NETDVR_FuHongUserNumber_t 
{
	struct NETDVR_userInfo_t userinfo[50];			//富翰50个用户
};
...
struct NETDVR_Framerate_list_t
{
	...
	unsigned char framerate[32];	//帧率列表 未使用的填0
};
...
#define VIDEORESOLU_2CIF			8
#define NETDVR_VIDEORESOLU_END		VIDEORESOLU_2CIF //在NETDVR_VIDEORESOLU_D1这里结束，可以通过判断是否在BEGIN和END之间来确认是否正确
...
struct  NETDVR_PTZProtocolList_t
{
	...
	unsigned char	PTZProtocollist[19];	//云台协议列表
};
...
struct NETDVR_UpnpParam_t  
{
	unsigned char flag_UPNP;                        //启用标志
	char upnp_id[64];                    //upnp id
	char upnp_passwd[64];            //upnp密码
	unsigned char reserved[3];
};

struct NETDVR_NTPParam_t{
	unsigned char enable;     //使能标志
	unsigned char time_zone;  //时区，界面上需要有两个选项，一个是时区类型：EAST\GMT\WEST三种，分别代表东、UTC、西时区，另一个是时区数值，范围是1-12。此成员的值要根据界面的设置来进行转化，例如：选GMT的时候，隐藏时区数值设置，此时传回来的time_zone的值为12，西时区传回的值的范围为0-11，东时区传回的时区值的范围为13-24。
	char ntp_server[25];      //ntp服务器地址，可以是域名或ip，长度限制最大24个字符
	unsigned char reserved[5];
};

struct NETDVR_ImageColor_t
{
	unsigned char	 chn;
	unsigned int  stime1;                //时间段1起始时间
	unsigned int  etime1;                //时间段1结束时间
	unsigned char   brightness1;            //时间段1亮度
	unsigned char   contrast1;               //时间段1对比度
	unsigned char   saturation1;            //时间段1饱和度
	unsigned char   hue1;                    //时间段1色调
	unsigned int  stime2;                //时间段2起始时间
	unsigned int  etime2;                //时间段2结束时间
	unsigned char   brightness2;            //时间段2亮度
	unsigned char   contrast2;               //时间段2对比度
	unsigned char   saturation2;            //时间段2饱和度
	unsigned char   hue2;                    //时间段2色调
	// modify by wait
	/*
	unsigned char reserved[15]; //预留
	*/
	unsigned char  acutance1;            //时间段1锐度
	unsigned char  acutance2;            //时间段2锐度
	unsigned char  reserved[13]; //预留
};


struct NETDVR_LTNetParam_t
{
	unsigned char flag_KeyNet;             //启用标志
	char KeyNet_id[24];         //立通网id信息
	unsigned char reserved[31];			//预留
};

struct NETDVR_LTNetReg_t
{
	char KeyNet_id[24];         //立通网id信息
	unsigned char reserved[32];			//预留
};

struct NETDVR_LTCloudParam_t
{
	unsigned char flag_LtCloud;            //启用标志
	char LtCloud_id[24];        //立通云id信息
	unsigned char reserved[31];			//预留
};

struct NETDVR_LTCloudReg_t
{
	char LtCloud_id[24];         //立通云id信息
	unsigned char reserved[32];			//预留
};

struct NETDVR_Video_Blackedging_t
{
	unsigned char chn;		//通道
    unsigned char horizon;// 水平调节  0-100 
    unsigned char vertical;//垂直调节  0-100
	unsigned int  copy2Chnmask;		//复制到其他通道。按位
	unsigned char reserved[5];
};

struct NETDVR_updateConfigParam_t 
{
	char filename[64];						//not neccessary
	unsigned int size;						//file size
...
int __stdcall NETDVR_SetRealPlayResolution(int nRealPlayHandle, char chn, char resolution);
...
int __stdcall NETDVR_SetPlayBackResolution(int nPlayBackHandle, char resolution);
...
int __stdcall NETDVR_setRecordParamsMd(int Handle, const struct NETDVR_recordParam_t *p_para);
int __stdcall NETDVR_getRecordParamsMd(int Handle, unsigned char chn, struct NETDVR_recordParam_t *p_para);

int __stdcall NETDVR_setRecordParamsAm(int Handle, const struct NETDVR_recordParam_t *p_para);
int __stdcall NETDVR_getRecordParamsAm(int Handle, unsigned char chn, struct NETDVR_recordParam_t *p_para);
...
*/
int __stdcall NETDVR_restoreFactorySettings(int Handle, int type);
...
int __stdcall NETDVR_setImageColor(int Handle, const struct NETDVR_ImageColor_t *p_para);
int __stdcall NETDVR_getImageColor(int Handle, unsigned char chn, NETDVR_ImageColor_t *p_para);
...
int __stdcall NETDVR_GetFuHongUserInfo(int Handle, struct NETDVR_FuHongUserNumber_t *p_para);
...
int __stdcall NETDVR_getBitRateListbyresolution( int Handle, unsigned char chn, unsigned char vidoetype, unsigned char resolution, NETDVR_bitRateList_t* p_para );
//xwserver
...
int __stdcall NETDVR_getFrameRateList2( int Handle, unsigned char chn, unsigned char videotype, unsigned char resolution, NETDVR_Framerate_list_t* pList );
...
int __stdcall NETDVR_getUpnpPara(int Handle, struct NETDVR_UpnpParam_t *p_para);
int __stdcall NETDVR_setUpnpPara(int Handle, const struct NETDVR_UpnpParam_t *p_para);

int __stdcall NETDVR_getNtpPara(int Handle, struct NETDVR_NTPParam_t *p_para);
int __stdcall NETDVR_setNtpPara(int Handle, const struct NETDVR_NTPParam_t *p_para);

int __stdcall NETDVR_CreateDvrByUpnp(int *p_Handle, char* name, char* password, char *upnpsvr, unsigned short upnpsvrport);

//syq
int __stdcall NETDVR_BindLTCloud(char* pUUID, unsigned short nPort, unsigned short &nVVPort);
int __stdcall NETDVR_CreateDvrByLTCloud(int *p_Handle, unsigned short nDevPort,unsigned short nMapPort,char* name, char *upnpsvr, unsigned short upnpsvrport);
int __stdcall NetDVR_GetRemoteIpPortByUpnp(char* name, char* password, char *upnpsvr, unsigned short upnpsvrport, unsigned int &remoteip, unsigned short &remoteport);

//获得立通网信息
int __stdcall NETDVR_getLTNetPara(int Handle, struct NETDVR_LTNetParam_t *p_para);
//设置立通网信息
int __stdcall NETDVR_setLTNetPara(int Handle, const struct NETDVR_LTNetParam_t *p_para);
//注册立通网
int __stdcall NETDVR_registLTNet(int Handle, struct NETDVR_LTNetReg_t *p_para);


//获得立通云信息
int __stdcall NETDVR_getLTCloudPara(int Handle, struct NETDVR_LTCloudParam_t *p_para);
//设置立通云信息
int __stdcall NETDVR_setLTCloudPara(int Handle, const struct NETDVR_LTCloudParam_t *p_para);
//注册立通云
int __stdcall NETDVR_registLTCloud(int Handle, struct NETDVR_LTCloudReg_t *p_para);

//获得通道黑边值
int __stdcall NETDVR_getVideoBlackedging(int Handle, unsigned char chn, NETDVR_Video_Blackedging_t *p_para);
//设置通道黑边值
int __stdcall NETDVR_setVideoBlackedging(int Handle, const struct NETDVR_Video_Blackedging_t *p_para);

// 配置文件上传下载
int NETDVR_regCBConfigUploadSendProgress(int Handle, PFUN_PROGRESS_T p_cb_func, unsigned int dwContent);
int NETDVR_regCBConfigUploadUpdateError(int Handle, PFUN_ERROR_T p_cb_err, unsigned int dwContent);
int NETDVR_startConfigUpload(int Handle, const struct NETDVR_updateConfigParam_t *p_update_para, /*unsigned short update_port, */char *s_update_dir, char *s_update_filename);
int NETDVR_stopConfigUpload(int Handle);
int NETDVR_IsConfigUpload(int Handle);
int NETDVR_regCBConfigDownloadProgress(int Handle, PFUN_PROGRESS_T p_cb_progress, unsigned int dwContent);
int NETDVR_regCBConfigDownloadError(int Handle, PFUN_ERROR_T p_cb_err, unsigned int dwContent);
int NETDVR_startConfigDownload(int Handle, char *s_save_dir, char *s_save_filename);
int NETDVR_stopConfigDownload(int Handle);
