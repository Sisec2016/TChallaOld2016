...
UINT32 StartMountProcess(const QString& strMountProcessPath , MountSecurityDiskParam aMountParam ,  CHAR &chMountLetter , CHAR chMaxLetter )
{
    g_hEventMount = CreateEvent(NULL,FALSE,FALSE,NULL);

    WCHAR chLetter = 0 ;
    UINT32 retValue = MountSecurityDiskW_SageAPI( strMountProcessPath.toStdWString().c_str(), aMountParam , chLetter , chMaxLetter ) ;
    if ( 0 != retValue )
    {
        return -1 ;
    }
    chMountLetter = (CHAR)chLetter ;
