...
namespace hprose
{
    ...
    public:
        ...
        void read_list(ValueType & a, bool include_tag = true)
        {
            ...
            for (int i = 0; i < count; i++)
            {
                a.push_back(unserialize<element>());
            }
        ...
        void read_object(Object& o, bool include_tag = true)
        {
            if (include_tag)
            {
            ...
            const std::string& type = classref[read_int(TagOpenbrace)];
            std::vector<std::string> & attrs = attrsref[type];
            references.push_back(&o);
            o.resize(attrs.size());
            for (size_t i = 0; i < attrs.size(); i++)
            {
                this->unserialize<any>(o[i], AnyType());
            }
    ...
    private:
        ...
        void unserialize(ValueType & a, AnyType)
        {
            ...
            switch (tag)
            {
                ...
                case TagRef:
                    {
                        int num = read_int(TagSemicolon);
                        if (num < references.size())
                        {
                            a = references[num];
                        }
                    }
    ...
    private:
        ...
        std::vector<std::string> classref;
        std::map<std::string, std::vector<std::string>> attrsref;
